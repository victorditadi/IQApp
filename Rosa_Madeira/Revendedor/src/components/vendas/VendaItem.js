...
class VendaItem extends Component {
  ...
  detalheProduto(id_venda){
    Actions.produto({id_venda: id_venda});
  }

  render() {
    const { id_cliente, status, data, numero_produto, id_venda, valor } = this.props.listVendas;
    const {
      headerContentStyle,
      thumbnailStyle,
      ...
      return(
        <Card>
          ...
          <CardSection>
            <Button onPress={() => this.detalheProduto(id_venda)}>
              Produtos
            </Button>
